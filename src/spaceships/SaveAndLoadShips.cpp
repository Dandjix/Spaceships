...
void SaveShip(const std::string &blueprint, const std::string& file_name)
{
    const char* filePath = tinyfd_saveFileDialog(
        ...
        (std::filesystem::path(Directories::get()->assets() /"spaceships") / file_name).c_str(),             // Default filename
...
std::string LoadShip(std::string * path)
{
    const char* filePath = tinyfd_openFileDialog(
        ...
        std::filesystem::path(Directories::get()->assets() /"spaceships").c_str(),                          // Initial directory (empty string means default)
