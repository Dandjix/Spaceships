...
namespace GUI {
    ...
    public:
        FramedLabel(Anchor anchor, const Vector2Int &offset, int width, int height, const std::string &labelText,
            const SDL_Color &labelColor, TTF_Font *labelFont, float margin = 5.0f, SDL_Color inner_color = {0,0,0,255}, SDL_Color outer_color = {255,255,255,255})
            : GUILabel(anchor, offset, width, height, labelText, labelColor, labelFont), padding(margin), inner_color(inner_color),outer_color(outer_color) {
        }
