...
void Painter::setMode(Mode newMode) {
    ...
    switch (mode) {
        case Mode::Off:
            ...
            break;
        case Mode::Regions:
            changeState(std::make_unique<StateRegionsIdle>());
            break;
        case Mode::Point:
            changeState(std::make_unique<StatePoint>());
            break;
    }
}
...
void Painter::confirmPlacement(const std::string &name) {
    if (mode == Mode::Point) {
        on_point_placed.emit(name, start_position);
    }
        ...
        on_region_placed.emit(name, TL, dim);
    }
